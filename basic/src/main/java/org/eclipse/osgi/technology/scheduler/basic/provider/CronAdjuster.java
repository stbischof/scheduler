...
class CronAdjuster implements TemporalAdjuster {
    ...
    /*
     * A function interface that we use to check a Temporal to see if it matches a
     * part of the specification. We combine these checkers in and and ors.
     */
    interface Checker {
    ...
    /*
     * Maintains the type and the combined checker. It can verify if a specific part
     * of the temporal is ok, and if not, it will reset it to the next higher
     * temporal with the lower fields set to their minimum value.
     */
    static class Field {
        ChronoField type;
        Checker checker;
    ...
    /*
     * We adjust the given temporal with this type before we check it to prevent it
     * from keep matching the same value.
     */
    ...
    private String preDeclared(String expression) {
        ...
        case Constants.CRON_EXPRESSION_ANNUALLY, Constants.CRON_EXPRESSION_YEARLY -> "0 0 0 1 1 *";
        case Constants.CRON_EXPRESSION_MONTHLY -> "1 0 0 1 * *";
        case Constants.CRON_EXPRESSION_WEEKLY -> "2 0 0 ? * MON";
        case Constants.CRON_EXPRESSION_DAILY -> "3 0 0 * * ?";
        case Constants.CRON_EXPRESSION_HOURLY -> "4 0 * * * ?";
        case Constants.CRON_EXPRESSION_MINUTELY -> "0 * * * * *";
        case Constants.CRON_EXPRESSION_SECONDLY -> "* * * * * *";
        case Constants.CRON_EXPRESSION_REBOOT -> "0 0 0 1 1 ? 1900";
        default -> expression;
        };
    }

    /*
     * A cron part consists of a number of sub expressions separated by a comma. We
     * parse each sub expression and combine the results.
     */
    ...
    /*
     * This is the # syntax. We must check that the given weekday is the nth one in
     * the current month. So we take the day of the month and divide it by 7.
     */
    ...
    /*
     * Check for the nearest working day. E.g. 15W is the nearest working day around
     * the 15th.
     */
